...
class Ncalayer:
    ...
    def extract_info(self, base64_signature: str) -> UserEcpInfo:
        """
        Извлекает информацию о пользователе из сертификата
        """

        content_info = self._load_signature(base64_signature)
        cert_der = self._get_cert_der(content_info)
    ...
    def check_signed_data(self, base64_signature: str, original_data: str) -> bool:
        content_info = self._load_signature(base64_signature)
        signature_value = self._get_signature_value(content_info)
